...
class LensAnalysesOnOctopress(object):
  def process_file(fname):
    """ 
	Check if _fname_ has EXIF information with focal length that was used in camera, when photo was taken.
	Return None, if focal length was not found, otherwise return focal length as string with "mm" suffix.
	"""
    fl = None
    f = open(fname)
    data = f.read(12)
  ...
  def map_filename(fname):
    """ Map filename in blog post to filename into original image with EXIF information. """
  ...
  def scan(dir):
    ...
    for fname in glob.glob("%s%s*.markdown" % (dir,os.sep)):
	  ...
	  """ Cut off date (yyyy-mm-dd) from beginning and '.markdown' from end of filenames. """
      status = 0
      title = None
      fl = None
      for line in f:
